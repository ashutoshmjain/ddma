...
def test_flow():
    ...
    # Add Content-Type to the form data fields
    upload_fields["Content-Type"] = "video/mp4"
    
    print(f"video_id: {video_id}")
    
    # 2. Upload to GCS
    print(f"Step 2: Uploading {video_file} to GCS with Content-Type form field...")
    with open(video_file, "rb") as f:
        files = {"file": (filename, f, "video/mp4")}
        res_gcs = requests.post(upload_url, data=upload_fields, files=files)
    print(f"GCS code: {res_gcs.status_code}")
    print(f"GCS body: {res_gcs.text}")
